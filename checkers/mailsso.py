"""
Checker for mails.so – with fallback for recaptcha token.
"""
import requests
import threading
...
CREDITS_URL = BASE_URL + "/user/credits"
TIMEOUT = 15
...
def check(email: str, password: str) -> Tuple[bool, str]:
    ...
    for token in tokens_to_try:
        ...
        try:
            ...
            # If we got an error message, store it and continue to next token
            msg = data.get("message", "")
            if msg:
                last_error = msg
                if "invalid" in msg.lower() or "incorrect" in msg.lower():
                    return False, f"Login failed: {msg}"
            # If no message, maybe invalid token – try next

        except Exception as e:
            last_error = str(e)
            continue

    # If we exhausted all tokens and still failed
    if last_error:
        return False, f"Login failed: {last_error}"
    return False, "Login failed – no valid response"
